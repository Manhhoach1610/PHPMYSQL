	...
	<div id="contents">
		<div id="contact" class="wrapper clearfix">
			...
			<div class="main">
				...
				<?php  
					require('control.php');
					$getdata = new Data(); // gọi đến class data đc gán bằng biến $getdata
					
					$check="/([a-z0-9_]+|[a-z0-9_]+\.[a-z0-9_]+)@(([az0-9]|[a-z0-9]+\.[a-z0-9]+)+\.([a-z]{2,4}))/i"; 
					if (!preg_match($regex,$email)) {
						echo "Địa chỉ mail phù hợp";
 					} else {
						 echo "Email khong đúng!!!";
					 } 
					if(isset($_POST['btn'])) {
						if(empty($_POST['txtEmail']) && empty($_POST['txtMessage'])) {
							...
							$_POST['txtMessage'], $_POST['txtTerm'], $_POST['txtSubcribe']);  // khai báo 1 biến insert với lớp data chỉ đến function in_contact  
							if($insert) { 
								// header('location:Trang_1.php'); // chuyển đến trang1.php
